...
def test_controlnet_cannyedge(prompt, save_path, canny_path, pretrained_path_canny, num_images_per_prompt=4,
...
def MARIOEval_generate_results(root, dataset, args, method='controlnet_seg', num_images_per_prompt=4, split=0, total_split=1):
    ...
    if method == 'controlnet_canny':
        pipe = load_controlnet('lllyasviel/sd-controlnet-canny')
    ...
    for idx, prompt in tqdm(enumerate(prompts)): 
        if idx < split * len(prompts) / total_split or idx > (split + 1) * len(prompts) / total_split:
            continue
        if  method == 'controlnet_canny':
            test_controlnet_cannyedge(prompt=prompt, pretrained_path_canny='lllyasviel/sd-controlnet-canny', num_images_per_prompt=num_images_per_prompt,
            ...
            test_controlnet_segmask(prompt=prompt, pretrained_path_seg='lllyasviel/sd-controlnet-canny', num_images_per_prompt=num_images_per_prompt,
                        save_path=os.path.join(root_res, dataset, 'images', str(idx) + '.jpg'),
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        choices=['controlnet_canny', 'controlnet_seg', 'deepfloyd', 'stablediffusion', 'textdiffuser']
    )
    parser.add_argument(
        "--gpu",
        type=int,
